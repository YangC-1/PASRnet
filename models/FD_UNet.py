...
def FD_UNet(input, filters=32, kernel_size=3, padding='same',
    ...
    out = Conv2D_BatchNorm(out, filters, kernel_size=kernel_size, strides=1, padding=padding,
                           activation=activation, kernel_initializer=kernel_initializer)
    out = FD_Block(out, f_in=filters, f_out=filters*2, k=filters//4, kernel_size=kernel_size,
...
def DownBlock(input, filters, kernel_size, padding, activation, kernel_initializer):
    ...
    out = FD_Block(input, f_in=filters//2, f_out=filters, k=filters//8, kernel_size=kernel_size,
                   padding=padding, activation=activation, kernel_initializer=kernel_initializer)
    shortcut = out
...
def UpBlock(input, filters, kernel_size, padding, activation, kernel_initializer):
    ...
    out = Conv2D_BatchNorm(input, filters=filters//2, kernel_size=1, strides=1, padding=padding,
                           activation=activation, kernel_initializer=kernel_initializer)
    out = FD_Block(out, f_in=filters//2, f_out=filters, k=filters//8, kernel_size=kernel_size,
                   padding=padding, activation=activation, kernel_initializer=kernel_initializer)
    out = UpSample(out, filters//2, kernel_size, strides=2,padding=padding,
                   activation=activation, kernel_initializer=kernel_initializer)
    ############################################
...
def getModel(input_shape, filters=32, kernel_size=3, padding='same',
             activation='relu', kernel_initializer='glorot_normal'):
    ...
    model_outputs = FD_UNet(model_inputs, filters=filters, kernel_size=kernel_size, padding=padding,
                            activation=activation, kernel_initializer=kernel_initializer)
    model = Model(model_inputs, model_outputs, name='FD_UNet_Model')
    return model
